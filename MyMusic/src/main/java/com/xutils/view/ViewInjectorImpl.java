...
import com.xutils.ViewInjector;
...
public final class ViewInjectorImpl implements ViewInjector {
    ...
    public void inject(Activity activity) {
        ...
        try {
            com.xutils.view.annotation.ContentView contentView = findContentView(handlerType);
            if (contentView != null) {
     ...
     * 从父类获取注解View
     */
    private static com.xutils.view.annotation.ContentView findContentView(Class<?> thisCls) {
        if (thisCls == null || IGNORED.contains(thisCls)) {
            return null;
        }
        com.xutils.view.annotation.ContentView contentView = thisCls.getAnnotation(com.xutils.view.annotation.ContentView.class);
        if (contentView == null) {
    ...
    private static void injectObject(Object handler, Class<?> handlerType, ViewFinder finder) {
        ...
        if (methods != null && methods.length > 0) {
            for (Method method : methods) {
                ...
                //检查当前方法是否是event注解的方法
                com.xutils.view.annotation.Event event = method.getAnnotation(com.xutils.view.annotation.Event.class);
                if (event != null) {
                    try {
                        // id参数
                        ...
                        for (int i = 0; i < values.length; i++) {
                            ...
                            if (value > 0) {
                                ...
                                com.xutils.view.EventListenerManager.addEventMethod(finder, info, event, handler, method);
                            }
                        }
                    } catch (Throwable ex) {
                        LogUtil.e(ex.getMessage(), ex);
