...
public final class LruDiskCache {
    ...
    public com.xutils.cache.DiskCacheEntity get(String key) {
        if (!available || TextUtils.isEmpty(key)) return null;

        com.xutils.cache.DiskCacheEntity result = null;
        try {
            result = this.cacheDb.selector(com.xutils.cache.DiskCacheEntity.class)
        ...
        if (result != null) {
            ...
            { // update hint & lastAccess...
                final com.xutils.cache.DiskCacheEntity finalResult = result;
    ...
    public void put(com.xutils.cache.DiskCacheEntity entity) {
        if (!available
                || entity == null
    ...
    public DiskCacheFile getDiskCacheFile(String key) throws InterruptedException {
        ...
        com.xutils.cache.DiskCacheEntity entity = get(key);
    ...
    public DiskCacheFile createDiskCacheFile(com.xutils.cache.DiskCacheEntity entity) throws IOException {
    ...
    /*package*/ DiskCacheFile commitDiskCacheFile(DiskCacheFile cacheFile) throws IOException {
        ...
        DiskCacheFile result = null;
        com.xutils.cache.DiskCacheEntity cacheEntity = cacheFile.cacheEntity;
        ...
        trimExecutor.execute(new Runnable() {
            ...
            public void run() {
                if (available) {
                    ...
                    // trim db
                    try {
                        int count = (int) cacheDb.selector(com.xutils.cache.DiskCacheEntity.class).count();
                        if (count > LIMIT_COUNT + 10) {
                            List<com.xutils.cache.DiskCacheEntity> rmList = cacheDb.selector(com.xutils.cache.DiskCacheEntity.class)
                                    .orderBy("lastAccess").orderBy("hits")
                                    .limit(count - LIMIT_COUNT).offset(0).findAll();
                            if (rmList != null && rmList.size() > 0) {
                                // delete cache files
                                for (com.xutils.cache.DiskCacheEntity entity : rmList) {
                                    try {
                                        // delete db entity
                                        cacheDb.delete(entity);
                                        // delete cache files
    ...
    private void deleteExpiry() {
        try {
            ...
            List<com.xutils.cache.DiskCacheEntity> rmList = cacheDb.selector(com.xutils.cache.DiskCacheEntity.class).where(whereBuilder).findAll();
            // delete db entities
            cacheDb.delete(com.xutils.cache.DiskCacheEntity.class, whereBuilder);
            if (rmList != null && rmList.size() > 0) {
                // delete cache files
                for (com.xutils.cache.DiskCacheEntity entity : rmList) {
                    String path = entity.getPath();
                    ...
                    try {
                        ...
                        if (fileList != null) {
                            for (File file : fileList) {
                                try {
                                    long count = cacheDb.selector(com.xutils.cache.DiskCacheEntity.class)
