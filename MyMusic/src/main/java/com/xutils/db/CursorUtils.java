...
/*package*/ final class CursorUtils {
    ...
    public static <T> T getEntity(com.xutils.db.table.TableEntity<T> table, final Cursor cursor) throws Throwable {
        T entity = table.createEntity();
        HashMap<String, com.xutils.db.table.ColumnEntity> columnMap = table.getColumnMap();
        int columnCount = cursor.getColumnCount();
        for (int i = 0; i < columnCount; i++) {
            String columnName = cursor.getColumnName(i);
            com.xutils.db.table.ColumnEntity column = columnMap.get(columnName);
            if (column != null) {
