 ...
 * @param <ResultType>
 */
public abstract class AbsTask<ResultType> implements com.xutils.common.Callback.Cancelable {

    private TaskProxy taskProxy = null;
    private final com.xutils.common.Callback.Cancelable cancelHandler;
    ...
    public AbsTask(com.xutils.common.Callback.Cancelable cancelHandler) {
    ...
    protected void onCancelled(com.xutils.common.Callback.CancelledException cex) {
    }
    ...
    public final synchronized void cancel() {
        if (!this.isCancelled) {
            ...
            if (this.state == State.WAITING || (this.state == State.STARTED && isCancelFast())) {
                if (taskProxy != null) {
                    taskProxy.onCancelled(new com.xutils.common.Callback.CancelledException("cancelled by user"));
                    taskProxy.onFinished();
                } else if (this instanceof TaskProxy) {
                    this.onCancelled(new com.xutils.common.Callback.CancelledException("cancelled by user"));
                    this.onFinished();
                }
            }
        }
