...
import com.xutils.common.TaskController;
...
public final class TaskControllerImpl implements TaskController {
    ...
    public <T> AbsTask<T> start(AbsTask<T> task) {
        com.xutils.common.task.TaskProxy<T> proxy = null;
        if (task instanceof com.xutils.common.task.TaskProxy) {
            proxy = (com.xutils.common.task.TaskProxy<T>) task;
        } else {
            proxy = new com.xutils.common.task.TaskProxy<T>(task);
        }
        try {
            proxy.doBackground();
        } catch (Throwable ex) {
    ...
    public <T extends AbsTask<?>> Callback.Cancelable startTasks(
        ...
        for (final T task : tasks) {
            start(new com.xutils.common.task.TaskProxy(task) {
                @Override
    ...
    public void autoPost(Runnable runnable) {
        ...
        if (Thread.currentThread() == Looper.getMainLooper().getThread()) {
            ...
            com.xutils.common.task.TaskProxy.sHandler.post(runnable);
        }
    }
    ...
    public void postDelayed(Runnable runnable, long delayMillis) {
        if (runnable == null) return;
        com.xutils.common.task.TaskProxy.sHandler.postDelayed(runnable, delayMillis);
    }
    ...
    public void run(Runnable runnable) {
        if (!com.xutils.common.task.TaskProxy.sDefaultExecutor.isBusy()) {
            com.xutils.common.task.TaskProxy.sDefaultExecutor.execute(runnable);
        } else {
    ...
    public void removeCallbacks(Runnable runnable) {
        com.xutils.common.task.TaskProxy.sHandler.removeCallbacks(runnable);
    }
}
