package com.xutils.common.util;
...
public final class ProcessLock implements Closeable {
    ...
    static {
        File dir = com.xutils.x.app().getDir(LOCK_FILE_DIR, Context.MODE_PRIVATE);
    ...
    private static ProcessLock tryLockInternal(final String lockName, final String hash, final boolean writeMode) {
        synchronized (LOCK_MAP) {
            ...
            try {
                File file = new File(
                        com.xutils.x.app().getDir(LOCK_FILE_DIR, Context.MODE_PRIVATE),
                        hash);
